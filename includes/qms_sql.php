...
function delete_espionnage($index)
{                                                    // Effacer un espionnage donné
    global $db;
...
function modify_espionnage($index)
{                                                    // Modifier un espionnage donné
    global $db, $user_data;
    static $result;
    if ($index != 0) {
...
function modify_all_espionnage($index_tab)
{                                            // Modifier plusieurs espionnage
    $indexx = explode('_x_', $index_tab);
    foreach ($indexx as $index)
        modify_espionnage($index);
    return 1;
}
function Interpolation($data, $nombre_mini, $periodes)
{                                // Procédure d'interpolation Joueur/Alliance (MERCI SANTORY!!!!!)
    global $db;
...
function analyse_espionnage($pub_nombre_mini, $pub_periodes)
{                        // Analyse de l'espionnage par planete (MERCI SANTORY!!!)
    global $db;
    global $user_data, $lang;
    static $tableau;
    $retour = "";
...
function get_tooltip_spy($spyer, $userid, $datadate, $JorA)
{                            // Renvoi un code de tooltip affichant un tableau listant les espionnages de $spyer sur $userid;
    global $db, $lang;
    static $spies;
...
function analyse_globale($periodes, $nb_rapport, $JorA)
{                                // Analyse des espionnages de TOUT le serveur.
...
function get_spies_string($position, $texte = "<color=red>E</color>")
{                    // Renvoi le lien vers le popup qui affiche les espionnages fait sur cette planete
    global $db, $result_2;
...
function get_moon_string($position)
{                                                // Renvoi le texte d'affiche d'une lune ou pas selon la position
    global $db;
    list($galaxy, $system, $row) = get_coord($position);
    $type = array("", "M", "M-3");
    return /*$type[intval(rand(0,2))]; //*/ "";
}
function delete_oldspies()
{                                                            // Efface les espionnages des joueurs qui n'ont plus de compte OGSpy
    global $db;
