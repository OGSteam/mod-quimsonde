...
function get_espionnage_count($userID = 0, $where = "", $filtre = "", $filtre_data = "")
{  // Renvoi le nombre total d'espionnage
    global $db;
...
function get_list($type, $private, $filtre_where = "")
{                             // Renvoi la liste de tous les $type different
    global $db, $user_data;
...
function get_most($data, $tableau)
{                                        //Renvoi de la valeur et le nombre de $data le plus trouvé
...
function get_list_order($tableau, $sort = "", $ord = "")
{ //Classer le tableau en fonction d'une donnée ($sort) et d'un sens ($ord)
...
function get_distance_moyen($tableau)
{                                          // Calcul de la distance moyenne des espionnages du $tableau
    $cnt = 0;
    $total = 0;
...
function get_pourcentage_moyen($tableau)
{                                       // Renvoi le pourcentage de destruction moyen du $tableau
    $cnt = 0;
    $total = 0;
...
function get_time_analyse($tableau)
{                                            // Renvoi la période sans espionnage la plus longue
    // retour :
...
{                                                // Renvoi une durée sous la forme de xj xh xm xs.
    $retour = "";
    $time -= 3600;
...
function get_color($poucentage, $mini = 0, $max = 100)
{                               // Renvoi la couleur en fonction du pourcentage (0% = blanc, 100% = rouge)
...
function get_clear_distance($distance)
{                                         // Renvoi le nombre de système, galaxie ou planetes correspondant à une distance
    global $lang;
    if ($distance >= 20000) {
        // Plusieurs galaxie
