    ...
    for ($i = 0; $i < count($pages); $i++) {
        ...
        if ($cfg_imgmenu == "yes") {
            if (($pages[$i]['admin'] && IsUserAdmin()) || (!$pages[$i]['admin']) && !$ignore_insertion) {
                ...
                if ($this_page) $ligne2 .= $styleON;
                else $ligne2 .= $styleOFF;
                if (!$this_page) {
...
function get_galaxy_link($position)
{                                // Renvoi le lien de recherche vers un systeme donné
    $tmp = get_coord($position);
    $ret = $tmp[0] . "&system=" . $tmp[1];
    return $ret;
}
function make_liste_string($type, $data, $private = 1)
{                 // Rempli la zone <select> des options de $type de filtrage
    global $lang;
    ...
    if (count($liste) > 0) {
        ...
        foreach ($liste as $option) {
            ...
            if ($data == $value) $selected = " selected";
            else $selected = "";
...
function get_tooltip($gtarget, $gtype, $from_popup = 0, $id = 0, $member = 0)
{    // Renvoi un code de tooltip inspiré de la page galaxie selon le Type "Joueur" ou "Alliance";
...
function make_spy_OGameStyle($table_spy, $j, $from_popup = 0)
{          // Affiche l'espionnage $j de la table $table_spy a la manière de la page message de OGame
    // Qui affiche?
