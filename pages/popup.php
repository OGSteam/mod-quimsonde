...
if (count($table_spy['id']) < 1) {
    echo sprintf($lang['qms_popup_spy_not_found'], $pub_target);
    exit();
}
...
if (isset($pub_choix_private) && $pub_choix_private == 'false') $private = false;
else $private = true;
...
function get_top_hour($tableau, $titre = "")
{
    ...
    foreach ($tableau['datadate'] as $id => $newdate) {
        ...
        if (isset($total_jour_[$day])) $total_jour_[$day] += 1;
        else $total_jour_[$day] = 1;
        if (isset($total_hour_[$hour])) $total_hour_[$hour] += 1;
        else $total_hour_[$hour] = 1;
        if (isset($total_jour_[7])) $total_jour_[7] += 1;
        else $total_jour_[7] = 1;
    }

    // Recherche des maximums et minimums
    foreach ($tab as $key_hour => $tab_hour) {
        foreach ($tab_hour as $key_day => $tab_day) {
            if (($a = $tab_day['cnt']) < $hit0) {
                $hit0 = $a;
                $lemoins = array($a, $key_hour, $key_day);
            }
            if ($a > $hit1) {
                $hit1 = $a;
                $leplus = array($a, $key_hour, $key_day);
            }
        }
    }
    $hit0 = 9999;
    ...
    $hit1 = 0;
    for ($i = 0; $i < 7; $i++) {
        if (isset($total_jour_[$i]) && $total_jour_[$i] < $hit0) {
            $hit0 = $total_jour_[$i];
            $totalJ_min = $total_jour_[$i];
        }
        if (isset($total_jour_[$i]) && $total_jour_[$i] > $hit1) {
            $hit1 = $total_jour_[$i];
            $totalJ_max = $total_jour_[$i];
        }
    }
    ...
    for ($i = 0; $i < 24; $i++) {
        ...
        if (isset($tab[$i])) {
            foreach ($tab[$i] as $k_day => $t_day) {
                ...
                if (count($spy_list) > 6) $height = "style=\"overflow-y:scroll;height:" . (6 * 25) . ";width:275\"";
                else $height = "";
